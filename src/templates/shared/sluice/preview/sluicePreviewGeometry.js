...
import { addBox, addGeometry } from './previewScene.js'
 ...
 * @param derived
 */
function buildGate(context, params, derived, showTrafficBridge) {
  ...
  addBox(
    ...
    cm(params.检修桥板宽),
    0,
    floorThick / 2 + pierHeight + cm(params.检修桥板厚) / 2,
    doorUpstreamZ,
  )
  ...
  addBox(
    ...
    cm(params.齿墙宽),
    0,
    -floorThick / 2 - cm(params.齿墙高) / 2,
    -length / 2 + cm(params.齿墙宽) / 2,
  )
}
/**
 * 消力池
 ...
 * @param derived
 */
function buildStilling(context, params, derived) {
  ...
  addBox(
    ...
    cm(params.消力坎顶宽),
    0,
    -slopeDrop + floorThick / 2 + cm(params.消力坎高) / 2,
    flatStartZ + cm(params.消力坎顶宽) / 2,
  )
  // 齿墙
  addBox(
    context,
    ...
    cm(params.消力池齿墙底宽),
    0,
    -slopeDrop - floorThick / 2 - cm(params.消力池齿墙高度) / 2,
    flatStartZ + cm(params.消力池齿墙底宽) / 2,
  )
}
/**
 * 连接段
 ...
 * @param part
 */
function buildConnection(context, params, derived, part) {
  const isUpstream = part === 'upstreamConnection'
  const length = cm(
    isUpstream ? params.上游连接段底板长度 : params.下游连接段底板长度,
  )
  const width = cm(
  ...
  const floorThick = cm(
    isUpstream ? params.上游连接段底板厚度 : params.下游连接段底板厚度,
  )
  const slopeThick = cm(
    isUpstream ? params.上游连接段坡板厚度 : params.下游连接段坡板厚度,
  )
  const pressWidth = cm(
    isUpstream ? params.上游连接段坡顶压坡宽 : params.下游连接段坡顶压坡宽,
  )
  const slopeToothThick = cm(
    isUpstream ? params.上游连接段坡齿厚度 : params.下游连接段坡齿厚度,
  )
  const slopeToothWidth = cm(
    isUpstream ? params.上游连接段坡齿宽度 : params.下游连接段坡齿宽度,
  )
  const toothHigh = cm(
    isUpstream ? params.上游连接段底板齿墙高 : params.下游连接段底板齿墙高,
  )
  const toothWide = cm(
    isUpstream ? params.上游连接段底板齿墙宽 : params.下游连接段底板齿墙宽,
  )
  const ditchWidth = cm(
    isUpstream ? params.上游连接段坡底槽宽 : params.下游连接段坡底槽宽,
  )
  const ditchHeight = cm(
    isUpstream ? params.上游连接段坡底槽高 : params.下游连接段坡底槽高,
  )
  const ditchOuterWidth = cm(
    isUpstream ? params.上游连接段坡底外槽宽 : params.下游连接段坡底外槽宽,
  )
 ...
 * @param part
 */
function buildTransition(context, params, derived, part) {
  ...
  const floorThick = cm(
    isUpstream ? params.上游渐变段断面铺盖厚 : params.下游渐变段断面铺盖厚,
  )
  ...
  addTransitionSlope(context, 'lower', backMiddle, lower, backMiddleZ, endZ, 1)
  const toeHeight = cm(
    isUpstream ? params.上游渐变段中断面趾高 : params.下游渐变段中断面趾高,
  )
  const toeWidth = cm(
    isUpstream ? params.上游渐变段中断面趾宽 : params.下游渐变段中断面趾宽,
  )
  const heelHeight = cm(
    isUpstream ? params.上游渐变段中断面踵高 : params.下游渐变段中断面踵高,
  )
  const topHeight = cm(
    isUpstream ? params.上游渐变段中断面顶端高 : params.下游渐变段中断面顶端高,
  )
  const topWidth = cm(
    isUpstream ? params.上游渐变段中断面顶端宽 : params.下游渐变段中断面顶端宽,
  )
  addBox(
    context,
    'toe',
  ...
  addBox(
    ...
    'tooth',
    lower.floorWidth,
    cm(isUpstream ? params.上游渐变段铺盖齿墙高 : params.下游渐变段铺盖齿墙高),
    cm(isUpstream ? params.上游渐变段铺盖齿墙宽 : params.下游渐变段铺盖齿墙宽),
    0,
    -floorThick / 2 -
      cm(
        isUpstream ? params.上游渐变段铺盖齿墙高 : params.下游渐变段铺盖齿墙高,
      ) /
        2,
    endZ,
  )
 ...
 */
function transitionSection(params, derived, isUpstream, section) {
