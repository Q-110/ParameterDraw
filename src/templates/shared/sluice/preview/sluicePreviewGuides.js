...
import { addDimensionLine } from './previewScene.js'
const upstreamConnectionKeys = {
  length: '上游连接段底板长度',
...
const downstreamConnectionKeys = {
  length: '下游连接段底板长度',
...
const downstreamTransitionKeys = {
...
// 用于做强制校验
const explicitGuideKeys = new Set([
  '闸室长',
  '闸孔净宽',
  '闸底板厚',
  '边墩厚',
 ...
 * @param derived
 */
function addGateGuides(context, params, derived, showTrafficBridge) {
  ...
  addXGuide(
    context,
    '门槽入闸墩深',
    firstOpeningX - openingWidth / 2 - cm(params.门槽入闸墩深),
    firstOpeningX - openingWidth / 2,
    topY,
    doorUpstreamZ,
    tickSize,
  ...
  addXGuide(
    ...
    firstOpeningX - doorWidth / 2 + cm(params.门槽二期宽),
    topY,
    slotDownstreamZ,
    tickSize,
  )
  addXGuide(
    context,
    '检修桥入闸墩深',
    -width / 2 + pierThick - cm(params.检修桥入闸墩深),
    -width / 2 + pierThick,
    topY + serviceThick,
    doorUpstreamZ,
    tickSize,
 ...
 * @param derived
 */
function addStillingGuides(context, params, derived) {
  ...
  addYGuide(
    ...
    '消力池齿墙高度',
    width / 2 + offset * 1.5,
    -slopeDrop - floorThick / 2 - cm(params.消力池齿墙高度),
    -slopeDrop - floorThick / 2,
    flatStartZ,
    tickSize,
  )
  ...
  addYGuide(
    ...
    width / 2 + lowerWallWidth + offset,
    -slopeDrop + floorThick / 2,
    -slopeDrop + floorThick / 2 + cm(params.消力池斜墙段高度),
    wallSectionZ,
    tickSize,
  )
  addYGuide(
    ...
    width / 2 + offset * 1.5,
    -slopeDrop + floorThick / 2,
    -slopeDrop + floorThick / 2 + cm(params.消力坎高),
    flatStartZ,
    tickSize,
  )
  addZGuide(
    ...
    '消力坎顶宽',
    width / 2 + offset,
    -slopeDrop + cm(params.消力坎高),
    flatStartZ,
    flatStartZ + cm(params.消力坎顶宽),
    tickSize,
  )
}
/**
 ...
 * @param isUpstream
 */
function addConnectionGuides(context, params, derived, keys, isUpstream) {
 ...
 * @param isUpstream
 */
function addTransitionGuides(context, params, derived, keys, isUpstream) {
  ...
  addYGuide(
    ...
    -floorThick / 2 - cm(params[keys.toothHeight]),
    -floorThick / 2,
    endZ,
    tickSize,
  )
  addZGuide(
    ...
    lower.floorWidth / 2 + offset,
    -floorThick / 2,
    endZ - cm(params[keys.toothWidth]) / 2,
    endZ + cm(params[keys.toothWidth]) / 2,
    tickSize,
  )
}
 ...
 * @param section
 */
function transitionGuideSection(params, derived, isUpstream, section) {
 ...
 * @param tickSize
 */
function addXGuide(context, key, startX, endX, y, z, tickSize) {
  addVectorGuide(
    context,
    key,
 ...
 * @param tickSize
 */
function addYGuide(context, key, x, startY, endY, z, tickSize) {
  addVectorGuide(
    context,
    key,
 ...
 * @param tickSize
 */
function addZGuide(context, key, x, y, startZ, endZ, tickSize) {
  addVectorGuide(
    context,
    key,
